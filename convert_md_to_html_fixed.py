...
def process_markdown_content(content):
    ...
    for para in paragraphs:
        ...
        # Handle image references first (before other processing)
        # Use a more specific regex to avoid conflicts
        para = re.sub(r'!\[([^\]]+)\]\(([^)]+)\)', 
        ...
        # Handle dialogue (text in quotes) - but avoid processing inside HTML tags
        # This is a simpler approach that should work better
        para = re.sub(r'(?<!<[^>]*)"([^"]+)"(?!\s*>)', r'<span class="dialogue">"\1"</span>', para)
