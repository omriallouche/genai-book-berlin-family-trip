...
def process_markdown_content(content):
    ...
    for para in paragraphs:
        para = para.strip()
        if not para or para.startswith('<h1'):
            processed_paragraphs.append(para)
            continue
            
        # Handle image references first
        para = re.sub(r'!\[([^\]]+)\]\(([^)]+)\)', 
        ...
        # Handle magic words (Hebrew words in italics)
        ...
        para = re.sub(r'^\*\*\*$', r'<hr style="border: none; height: 2px; background: linear-gradient(90deg, var(--primary-color), var(--secondary-color)); margin: 2rem 0; border-radius: 1px;">', para)
